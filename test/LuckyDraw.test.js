...
describe('LuckyDraw', async function () {
	...
	*/
	...
	it('test one participant to enter lucky draw games', async function () {
		...
		const total_participants = await LuckyDrawInstance.methods.total_participants().call({
			from : accounts[0]
		});
		...
		const contractBalance = await LuckyDrawInstance.methods.contractBalance().call({
			from : accounts[0]
		});
	...
	it('find a Winner', async function () {
		...
		if( winner == participant_1.participant_address ){
			...
			const new_balance_of_participant_1 = await web3.eth.getBalance(accounts[1]);

			assert( parseInt(balance_of_participant_1) < parseInt(new_balance_of_participant_1) );

		}else if( winner == participant_2.participant_address ){
			
			const new_balance_of_participant_2 = await web3.eth.getBalance(accounts[2]);

			assert( parseInt(balance_of_participant_2) < parseInt(new_balance_of_participant_2) );
		}
		...
		const totalParticipants = await LuckyDrawInstance.methods.total_participants().call({
			from : accounts[0]
		});
	...
	it('Destroy Contract', async function () {
		...
		const new_balance_of_manager_account = await web3.eth.getBalance(accounts[0]);

		// console.log(new_balance_of_manager_account);

		assert( parseInt(manager_account) < parseInt(new_balance_of_manager_account) );
	});
